...
if isfolder([inputFolderName '_merged.LFP'])                            % checks if there is any folder/file with .spike extension - Ideally, at this point, such file doesn't exist in the .rec folder unless Spikes were extracted using Trodes software GUI. 
    lfpFilePattern = dir([inputFolder '\*merged.LFP']);              % Creates a variable(spikeFilePattern) that lists files and folders in the current folder; check 'dir' function details
    cd([inputFolder '\' lfpFilePattern.name]);                          % changes the current directory to the name defined by variables/characters inside the sqaure bracket
    ...
    matFolder = mkdir([inputFolderName '_merged.LFP.mat']);             % Creates a new fodler whose name is a combination of strings defined by variable 'recFilename' ending with'.spikes.mat'
    ...
    for k = 1:numFiles                                                     % defines the k number of iterations to execute the 'for' loop                           
       cd([inputFolder '\' lfpFilePattern.name]); 
       ...
       fulldatFileName = fullfile(inputFolder, lfpFilePattern.name, baseFileName); % Concatenates and saves the full path of the 'baseFileName' name as variable 'fulldatFileName'
       ...
       fullmatFilePath = fullfile([inputFolder '\' inputFolderName '.LFP.mat' '\' datFileName,'.mat']); % Concatenates and saves the full path of the 'baseFileName' name as variable 'fullmatFilePath'
    ...
    addpath(genpath(inputFolder))                                              % adds the input folder with all subfolders to the matlab path in clusing any new folder created since addpath last time
    lfpFilePattern = dir([inputFolder '\*merged.LFP']);
    cd([inputFolder '\' lfpFilePattern.name]);
    ...
    [matFolder, matFileName, extension] = fileparts([recFileName '.LFP.mat']);
    ...
    for k = 1:numFiles
        ...
        [datFolder, datFileName, extension] = fileparts(fulldatFileName);
        fullmatFilePath = fullfile([inputFolder '\' recFileName '.LFP.mat' '\' datFileName,'.mat']);
        fprintf(1, 'Now reading file #%d of %d: %s to %s\n', ...
           k,numFiles , baseFileName, fullmatFilePath);
